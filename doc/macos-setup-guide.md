...
- `git clone git@github.com:computoma/dotfiles` and  run
  `./hosts/<hostname>/install.sh`.  Manual password input will be required a
...
- Press `Cmd+Shift+5`, pick _Options_ and set `~/Documents/Captures` as the
  storage location.
- Setup ProtonMail, GitHub, ChatGPT, WhatsApp, Spotify, YouTube, Fastmail, and
  Brave Talk on Safari.
- Setup GitHub, ChatGPT, Spotify, YouTube and Brave Talk on Brave.
